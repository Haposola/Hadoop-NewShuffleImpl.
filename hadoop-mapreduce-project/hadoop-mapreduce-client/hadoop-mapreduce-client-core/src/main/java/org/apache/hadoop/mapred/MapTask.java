...
import org.apache.hadoop.mapreduce.newShuffleImpl.MapOutputCollectAndPush;
...
public class MapTask extends Task {
  ...
  createSortingCollector(JobConf job, TaskReporter reporter)
    ...
    split = getSplitDetails(new Path(splitMetaInfo.getSplitLocation()),
            splitMetaInfo.getStartOffset());
    LOG.info("Processing split: " + split);
    MapOutputCollector.Context context =
            new MapOutputCollector.Context(this, job, reporter);
    Class<?>[] collectorClasses;
    if (job.getBoolean("mapreduce.shuffle.use.new.impl", false)) {
      collectorClasses = new Class<?>[]{MapOutputCollectAndPush.class};
      LOG.info("Seeing the right process YOYOYO");
    } else {
      collectorClasses = job.getClasses(
  ...
  void runNewMapper(final JobConf job,
    ...
    try {
      ...
      if (mapperContext.getConfiguration().getBoolean("mapreduce.shuffle.use.new.impl", false)) {
        Thread t = ((MapOutputCollectAndPush) (((NewOutputCollector) output).getOutputCollector())).getPushThread();
        t.join();
      }
  ...
  private class NewOutputCollector<K, V>
    ...
    public MapOutputCollector getOutputCollector() {
      return collector;
    }
